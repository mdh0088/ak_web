...
//        return cal.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY || cal.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY;
    ...
    private static String getConvDD(int dd)
    {
    	String a = Integer.toString(dd);
    	if(a.length() == 1)
    	{
    		a = "0"+a;
    	}
    ...
    public static ArrayList<String> getHoliday(String yoil) {
    	Date now = new Date();
    	SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
    	...
    	ArrayList<String> list = new ArrayList<String>();
    	 ...
    	 try 
    	 {
    		 for(int i = 1; i <= 12; i++)
    		 {
    			 for(int j = 1; j <= 31; j++)
    			 {
    				 String date = year+getConvDD(i)+getConvDD(j);
    				 Calendar cal = Calendar.getInstance();
    				 cal.setTime(sdf.parse(date));
    				 if(HolidayUtil.isHoliday(date) && Integer.toString(cal.get(Calendar.DAY_OF_WEEK)).equals(yoil))
    				 {
    					 list.add(year+getConvDD(i)+getConvDD(j));
    				 }
    			 }
    		 }
         } 
    ...
    public static ArrayList<String> getHoliday(String s_date, String e_date) {
    	Date now = new Date();
    	SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
    	String year = s_date.substring(0,4);
    	String year2 = e_date.substring(0,4);
    	ArrayList<String> list = new ArrayList<String>();
    	...
    	try 
    	{
    		for(int i = 1; i <= 12; i++)
    		{
    			for(int j = 1; j <= 31; j++)
    			{
    				...
    				Calendar cal = Calendar.getInstance();
    				cal.setTime(sdf.parse(date));
    				if(HolidayUtil.isHoliday(date) && Integer.parseInt(date) >= Integer.parseInt(s_date) && Integer.parseInt(date) <= Integer.parseInt(e_date))
    				{
    					if(!"20200530".equals(year+getConvDD(i)+getConvDD(j)))
    					{
    						list.add(year+getConvDD(i)+getConvDD(j));
    					}
    				}
    		...
    		if(!year.equals(year2))
    		{
    			for(int i = 1; i <= 12; i++)
    			{
    				for(int j = 1; j <= 31; j++)
    				{
    					String date = year2+getConvDD(i)+getConvDD(j);
    					Calendar cal = Calendar.getInstance();
    					cal.setTime(sdf.parse(date));
    					if(HolidayUtil.isHoliday(date) && Integer.parseInt(date) >= Integer.parseInt(s_date) && Integer.parseInt(date) <= Integer.parseInt(e_date))
    					{
    						if(!"20200530".equals(year2+getConvDD(i)+getConvDD(j)))
        					{
        						list.add(year2+getConvDD(i)+getConvDD(j));
        					}
    					}
    				}
    			}
