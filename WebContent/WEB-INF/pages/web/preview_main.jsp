	...
	$.ajax({
		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				...
				{
					inner += '<div class="rec-box swiper-slide">';
					...
					inner += '		<img src="${image_dir}wlect/'+result.list[i].THUMBNAIL+'" alt="이미지"/>';
					inner += '	</div>';
					inner += '	<p class="rec-stit">Watercolor Starter Class</p>';
								...
								<div class="swiper-slide">
									...
									<c:if test="${i.CON_TYPE eq 'file'}">
										<div class="slide-img" onclick="javascript:location.href='${image_dir}main_banner/${i.ATTACH}'" >
											<img src="${image_dir}main_banner/${i.BANNER}" alt="슬라이드이미지" style="width:1066px; height:880px;"/>
										</div>
									</c:if>
								...
								<div class="msear-bar table">
									...
									<div>
										...
										<div class="search-box search-box-two">
											...
											<div class="li-dep">
												<select id="sub_target" name="sub_target" de-data="강좌 분야를 선택해주세요.">
									...
									<div class="msear-div05">
										<div class="search-box">
											...
											<div class="input-sear">
												<input type="text" id="search_val" nmae="search_val" placeholder="#봄 제철재료 브런치 플래터" />
											</div>
										</div>
									</div>
								</div>
				...
				<div class="main-sec03 ani-eff text">
					...
					<div class="ani-eff image">
						<c:if test="${sbList[0].BANNER_POP eq 'now'}">
							<img src="${image_dir}sub_banner/${sbList[0].BANNER}" alt="혜택배너" onclick="javascript:location.href='${image_dir}sub_banner/${sbList[0].DETAIL}'" style="width:1284px; height:238px;"/>
						</c:if>
						<c:if test="${sbList[0].BANNER_POP eq 'new'}">
							<img src="${image_dir}sub_banner/${sbList[0].BANNER}" alt="혜택배너" onclick="javascript:window.open('${image_dir}sub_banner/${sbList[0].DETAIL}');" style="width:1284px; height:238px;"/>
						</c:if>
					</div>
				</div>
				<!-- //main-sec03 -->
		...
		<div id="main_popup" class="main_popup pop_idx${i.SEQ}" style="position: absolute; z-index: 10000; top: ${i.MARGIN_TOP}px; left: ${i.MARGIN_LEFT}px; display:none;">
			<c:if test="${i.POPUP_POP eq 'now'}">		
				<div class="" onclick="javascript:location.href='${i.POPUP_LINK}'">
					<img src="${image_dir}popup/${i.POPUP_IMG}" alt="" />
				</div>
			</c:if>
