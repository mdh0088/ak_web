...
function addAward(){
	...
	var inner='';
	inner +='<div id="award_area_'+cnt+'" class="career-wr">';
	inner +='	<div class="table">';
	inner +='		<div>';
	inner +='			<input type="text" id="award_date_'+cnt+'" class="date-i cal-inp" name="award_date" value="">';
	inner +='		</div>';
	inner +='	</div>';
	inner +='	<div class="teble">';
	inner +='		<div>';
	inner +='			<input type="text" id="award_from_'+cnt+'" class="inp-m10" name="award_from" placeholder="발행 기관을 입력해주세요.">';
	inner +='		</div>';
	inner +='	</div>';
	inner +='	<div class="inp-btn table">';
	inner +='		<div>';	
	inner +='			<input type="text" id="award_detail_'+cnt+'" name="award_detail" placeholder="수상내역을 입력해주세요.">';
	inner +='		</div>';
	inner +='		<div class="wid-3">';
	inner +='			<div>';
...
function addCert(){
	...
	var inner='';
	inner +='<div id="cert_area_'+cnt+'" class="career-wr">';
	inner +='	<div class="table">';
	inner +='		<div>';
	...
	inner +='	<div class="table">';
	inner +='		<div>';
	inner +='			<input type="text" id="cert_from_'+cnt+'" class="inp-m10" name="cert_from" placeholder="발행 기관을 입력해주세요.">';
	inner +='		</div>';
	inner +='	</div>';
	inner +='	<div class="inp-btn table">';
	inner +='		<div>';
	inner +='			<input type="text" id="cert_detail_'+cnt+'" name="cert_detail" placeholder="자격증명을 입력해주세요.">';
	inner +='		</div>';
	inner +='		<div class="wid-3">';
	inner +='			<div class="td-btn table">';
	inner +='				<div>';
...
function save(way){
	...
	if (f.school_yy_s.value > f.school_yy_e.value) {
		alert("재학기간을 수정해주세요.");
		f.school_yy_e.focus();
		f.school_yy_e.value('');
		return;
	...
	for (var i = 0; i < len; i++) {
		...
		if ($('input[name="s_career"]')[i].value > $('input[name="e_career"]')[i].value) {
			alert("경력 날짜를 수정해주세요.");
			$('input[name="e_career"]')[i].focus();
			$('input[name="e_career"]')[i].value('');
			return;
		}
...
<div class="lect-sec bg-gray">
	<div class="mu-grid">
		...
		<form name ="recruit" action='/mobile/academy/save' method='post' enctype="multipart/form-data">
			...
			<div class="lect-wr03">
				...
				<div class="row-wr">
					<div class="row">
						<p class="row-tit">사진 업로드</p>
						<div class="table img_file">
							<div>
					...
					<div class="row">
						<p class="row-tit">성명</p>
						<div><input type="text" id="name" class="inp100" value="${skor_nm}" disabled></div>
					</div>
					<div class="row">
						<p class="row-tit">휴대폰</p>
						<div>
							<input type="text" id="name" class="inp100" value="${sh_phone_no_1}-${sh_phone_no_2}-${sh_phone_no_3}" disabled>
						</div>
					</div>
					<div class="row">
						<p class="row-tit">생년월일</p>
						<div>
							<input type="text" id="name" class="inp100" value="${fn:substring(sbirth_ymd,0,4)}년 ${fn:substring(sbirth_ymd,4,6)}월 ${fn:substring(sbirth_ymd,6,8)}일" disabled>
						</div>
					</div>
					<div class="row">
						<p class="row-tit">이메일</p>
						<div><input type="text" id="email" class="inp100" value="${semail_addr}" disabled></div>
					</div>
	
					<div class="row">
						<p class="row-tit">결혼유무</p>
						<div>		
							<ul class="td-chk">
					...
					<div class="row">
						<p class="row-tit">블로그/SNS URL</p>
						<div><input type="text" id="sns" class="inp100" name="sns_url" value="${sns_url}" placeholder="URL 입력"></div>
					</div>
			...
			<div class="lect-wr03">
				...
				<div class="row-wr">
					<div class="row">
						<p class="row-tit">최종학력</p>
						<div class="td-school table">
							<div>
						...
						<div class="table">
							<div>
								<input type="text" name="major" maxLength="10" value="${SCH_MAJOR}" placeholder="내용을 입력해주세요.">
							</div>
						</div>
					</div>
					<div class="row">
						<p class="row-tit">학교소재지</p>
						<div class="td-school">
							<input type="text" name="school_from" value="${SCH_PLACE}"  placeholder="소재지를 입력해주세요."> 
						</div>
					</div>
					<div class="row">
						<p class="row-tit">재학기간</p>
						<div class="table">
							<div>
			...
			<div class="lect-wr03">
				...
				<div class="row-wr">
					<div class="row">
						<p class="row-tit">경력사항</p>
						<div id="target_career">
					...
					<div class="row">
						...
						<div>
							...
							<div class="table">
								<div>
									<input type="text" class="inp-m10" name="award_from" value="${AWAD1_FROM}" placeholder="발행 기관을 입력해주세요.">		
								</div>
							</div>
							<div class="inp-btn table">
								<div>
									<input type="text" name="award_detail" value="${AWAD1_DETAIL}" placeholder="수상내역을 입력해주세요.">
								</div>
								<div class="wid-3">
									<div class="td-btn table">
										<div>
					...
					<div class="row">
						...
						<div id="target_cert">
							<div>
								...
								<div class="table">
									<div>
										<input type="text" class="inp-m10" name="cert_from" value="${CERT1_FROM}" placeholder="발행 기관을 입력해주세요.">	
									</div>
								</div>
								<div class="inp-btn table">
									<div>
										<input type="text" name="cert_detail" value="${CERT1_DETAIL}" placeholder="자격증명을 입력해주세요.">
									</div>
									<div class="wid-3">
										<div class="td-btn table">
											<div>
